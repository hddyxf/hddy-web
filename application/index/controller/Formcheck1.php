...
class Formcheck1 extends Controller
{
    /*$data为你要查重的数据
     * $table为你要查重的表
     * $checkey为你要查重的字段
...
//        return $data;
//    }
    public function limit_select_user($username,$limit_table,$limit_data1,$limit_data2,$sid){
//        return array($username,$limit_table,$limit_data1,$sid,$limit_data2);
        $limit=Db::name($limit_table)
            ->where('username',$username)
            ->find();
//        return $limit;
        $res=Db::name('stu_view')
            ->where('s_id',$sid)
            ->where($limit_data1,$limit[$limit_data2])
            ->find();
//        return array($res,$username,$limit_table,$limit_data1,$sid,$limit_data2,$limit[$limit_data2]);
        return $res;
    }
}